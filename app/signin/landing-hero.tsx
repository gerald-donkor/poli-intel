...
/**
 * A Server Component. The scroll narrative lives in `landing-motion.tsx`, which
 * wraps the page and reaches these targets through their `data-anim` hooks —
 * so no section has to become a client component to hold a `useGSAP()` call.
 */
export function LandingHero({ alert }: LandingHeroProps) {
  return (
    <section data-anim="hero" className="relative isolate overflow-hidden">
      ...
      <ContourField
        ...
        className="pointer-events-none absolute -top-24 right-0 -z-10 h-[560px] w-full max-w-[900px] text-sage/50 [mask-image:radial-gradient(65%_60%_at_70%_35%,black,transparent_100%)] laptop:h-[680px]"
      />
      ...
      <div className="mx-auto w-full max-w-[1440px] px-4 py-10 tablet:px-8 tablet:py-16 laptop:py-20">
        <div className="grid grid-cols-1 items-center gap-10 laptop:grid-cols-12 laptop:gap-14">
          ...
          <div className="flex flex-col gap-6 laptop:col-span-7">
            {/* Institutional Badge */}
            <div
              data-anim="hero-badge"
              className="inline-flex w-fit items-center gap-2 rounded-full border border-surface-tint-border bg-surface-tint px-3 py-1 text-meta font-medium text-surface-tint-ink"
            >
              <span
                aria-hidden="true"
            ...
            {/* Heading. No `text-balance` here: it interferes with SplitText's
                line splitting, so the measure is capped instead. */}
            <div className="flex flex-col gap-3">
              <h1
                data-anim="hero-title"
                className="split-heading max-w-[16ch] text-display font-semibold tracking-tight text-ink tablet:max-w-[20ch] tablet:text-[40px] tablet:leading-[1.15]"
              >
                Making landscape knowledge work for forest policy.
              </h1>
              <p
                data-anim="hero-lede"
                className="split-heading max-w-[620px] text-body text-ink-2 tablet:text-[16px] tablet:leading-relaxed"
              >
          ...
          <div className="laptop:col-span-5">
            <div
              ...
              className="relative flex w-full flex-col gap-5 rounded-card border border-line bg-card p-6 shadow-overlay tablet:p-8"
            >
              <div className="flex flex-col gap-1.5">
                <div className="flex items-center gap-2.5">
                  {/* Abstract structural mark: concentric contour rings, inline
                      with the eyebrow rather than eating height above it. */}
                  <span
                    aria-hidden="true"
